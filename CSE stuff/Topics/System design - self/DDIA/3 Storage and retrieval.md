...
#insight its a trade off
-> good indexes speed up reads, but slow down writes. (::Consider adding: Clustered Index (actual data stored in index) vs. Non-Clustered/Secondary Index (contains pointer/PK to data).::)
...
[[Key Value store]] pair, each key is mapped to a position in the hashmap.

for value x, check memory offset, seek that offset, read.
bitcask is essentially this^

#insight its in memory, so we must ensure the hashmap is small enough to fit in the RAM. the data itself could be bigger than the RAM, because we can always make a disk read request.
...
advantages over has indexes (::typo: hash indexes::)
...
B-trees feature wide nodes with multiple children and keys, minimizing disk seeks by packing more data per block during reads or writes. This structure suits [[Database]]s and file systems like SQLite, where data exceeds RAM and access times dominate performance. They reduce tree height and I/O operations compared to binary trees on disk.[](https://stackoverflow.com/questions/2734692/avl-tree-vs-b-tree)

## AVL Trees for Memory

AVL trees are self-balancing binary search trees that prioritize fast lookups, insertions, and deletions with strict O(log n) height via rotations, ideal for RAM-resident data. They appear in scenarios like [[Database]] indexing or in-memory [[Caching]] needing predictable performance, though they use extra space for balance factors. Red-black trees often replace them in libraries for slightly better balance flexibility.[](https://www.geeksforgeeks.org/dsa/introduction-to-avl-tree/)​

---

One catch - because we are using an in memory datastructure (AVL etc), if the system crashes, then that memory is lost. so we should write through to disk as well (does not need to be sorted etc, just log it because we just care about recovery) (::Note: B-Trees also use a WAL (Write-Ahead Log) because they overwrite pages on disk; the WAL prevents corruption during partial page writes or splits.::)
...
#note lucene, the search engine used in [[elastic search]], uses something similar to LSM storage
...
- L0 will have the latest data that was flushed recently from memtable. search in L0, it will have duplicates as it is not compacted yet. (::Clarification: A single SSTable file has no duplicates, but the same key can exist in multiple different L0 files.::)
...
![[Pasted image 20251231000316.png]] (::B-Trees are "update-in-place" structures involving random writes, whereas LSM-trees turn random writes into sequential appends.::)
...
- **Why it works:**
  ...
  1. **I/O Efficiency:** If you only need to calculate `SUM(price)`, you only read the `price` column file from disk, skipping names, IDs, and emails. (::Crucial detail: For this to work, the row order must be identical across all separate column files to reconstruct the data correctly.::)
...
To make massive queries even faster, [[Database]]s use:
